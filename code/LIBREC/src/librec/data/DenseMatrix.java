 ...
 * 
 * A big reason that we do not adopt original DenseMatrix from M4J library is because the latter using one-dimensional
...
public class DenseMatrix implements Serializable {
	 ...
	 * Do {@code matrix x vector} between current matrix and a given vector
	 * 
	 * @return a dense vector with the results of {@code vector x Matrix}
	 */
	public DenseVector mult(DenseVector vec, boolean vecMat) {
		if (vecMat == false) {
			return mult(vec);
		} else {
	...
	/** MINE
	 * @return the sum of square of all data entries
	 */
	public double fro_Norm() {
		double res = 0;
		for (int row = 0; row < numRows; row++) {
			for (int col = 0; col < numColumns; col++) {
	...
	public void print(DenseMatrix mat)	{
		int nr = numRows;
		int nc = numColumns;
