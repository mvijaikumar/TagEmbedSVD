...
public class NMF extends IterativeRecommender {
	...
	public NMF(SparseMatrix trainMatrix, SparseMatrix testMatrix, SparseMatrix validMatrix, int fold) {
	...
	protected void buildModel() throws Exception {
		for (int iter = 1; iter <= numIters; iter++) {
			...
			if ((iter>5)|| iter == numIters)			
				minmet = evalRatMetrics(bw, "  Dim : " +Integer.toString(numFactors) +  "  filename : " + cf.getPath("trainpath") +"  ",minmet, iter==numIters); //mine            
 		}
		bw.close();
	}
