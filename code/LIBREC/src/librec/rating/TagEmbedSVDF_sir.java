...
public class TagEmbedSVDF_sir extends TagRecommender {
	...
	public TagEmbedSVDF_sir(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
		super(trainMatrix, testMatrix, fold);
	}

	public TagEmbedSVDF_sir(SparseMatrix trainMatrix, SparseMatrix testMatrix, SparseMatrix validMatrix, int fold)  throws Exception {
	...
	protected void initModel() throws Exception {
		...
		for (int u = 0; u < numUsers; u++) {
			...
			reg_Tu_vec.set(u, count > 0 ? 1.0 / count : 1.0);
		}
		...
		for (int j = 0; j < numItems; j++) {
			...
			reg_Tj_vec.set(j, count > 0 ? 1.0 / count : 1.0);
		}
		...
		for (int u = 0; u < numUsers ; u++) {
			...
			if (user_tags.size() > 0) {
				for (int t : user_tags) {
					//System.out.println("tag : " + userTagDao.getItemId(t));
	...
	protected void buildModel() throws Exception {		
		...
		for (int iter = 1; iter <= numIters; iter++) {
			loss = 0;
			//lRate = loc_lrate; loc_lrate = loc_lrate * 0.99;
