...
import librec.rating.TagCDCF;
import librec.rating.TagEmbedSVD;
import librec.rating.TagEmbedSVDF;
import librec.rating.TagEmbedSVDF_sir;
import librec.rating.TagEmbedSVD_sir;
import librec.rating.TagGSVDpp;
import librec.rating.TagTrustSVD;
...
public class LibRec {
	...
	public void execute(String[] args) throws Exception {
		System.out.println("I am inside execute");
	...
	private void runAlgorithm(LineConfiger evalOptions) throws Exception {
		...
		case "test-set":
			...
			DataDAO validDao = new DataDAO(evalOptions.getString("-vv"), rateDao.getUserIds(), rateDao.getItemIds()); //mine
			testDao.setTimeUnit(timeUnit);

			SparseMatrix[] testData = testDao.readData(columns, binThold);
			SparseMatrix[] validData = validDao.readData(columns, binThold); //mine
			data = new SparseMatrix[] { rateMatrix, testData[0], validData[0] };
			Recommender.testTimeMatrix = testData[1];
			//Recommender.validTimeMatrix = validData[1];
			break;
		case "given-n":
		...
		printEvalInfo(algo, algo.mesvalid,"valid");
		System.out.println("test error : \n");
		printEvalInfo(algo, algo.measures,"test");
		
		if (algo.algoName.contains("UserKNN") || algo.algoName.contains("ItemKNN") || algo.algoName.contains("GlobalAvg")) {
			System.out.println("Inside algo gl uk ik");
			String resFile = cf.getString("res.file", "./resFile.txt");
			BufferedWriter bw = new BufferedWriter(new FileWriter(resFile,true));
			String paramstr = "  filename : " + cf.getPath("trainpath");
			if (!algo.algoName.contains("GlobelAvg")) {
				paramstr = "shrinkage: " + cf.getInt("num.shrinkage", 30) +" neighbours: " + cf.getInt("num.neighbors", 20) + "  filename : " + cf.getPath("trainpath");
			}
			bw.write("\n"+algo.algoName +  "  " + paramstr +"  valmae:" + String.format("%.4f", algo.mesvalid.getRatingMetric("MAE").getValue()) + " testmae: "+ String.format("%.4f", algo.measures.getRatingMetric("MAE").getValue()) + "  valrmse:" + String.format("%.4f", algo.mesvalid.getRatingMetric("RMSE").getValue())+ "  testrmse:"+ String.format("%.4f", algo.measures.getRatingMetric("RMSE").getValue()));
			bw.close();
		}
     ...
     * print out the evaluation information for a specific algorithm
     */
    private void printEvalInfo(Recommender algo, MetricCollection ms, String set) throws Exception {
        ...
        // double commas as the separation of results and configuration
        StringBuilder sb = new StringBuilder();
        sb.append(set + "Metrics: ").append(ms.getMetricNamesString()).append("\n"); //added set
        String config = algo.toString();
        sb.append(algo.algoName).append(",").append(result).append(",,");
        if (!config.isEmpty())
            sb.append(config).append(",");
	...
	protected Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
		...
		SparseMatrix trainMatrix = data[0], testMatrix = data[1], validMatrix = data[2]; //mine
		...
		/* baselines */
		case "globalavg":
			return new GlobalAverage(trainMatrix, testMatrix, validMatrix, fold);
		case "useravg":
			return new UserAverage(trainMatrix, testMatrix, validMatrix, fold);
		case "itemavg":
			return new ItemAverage(trainMatrix, testMatrix, validMatrix, fold);
		case "usercluster":
		...
		case "mostpop":
			...
			/* rating prediction */
		case "userknn":
			return new UserKNN(trainMatrix, testMatrix, validMatrix, fold);
		case "itemknn":
			return new ItemKNN(trainMatrix, testMatrix, validMatrix, fold);
		case "itembigram":
		...
		case "svd++":
			return new SVDPlusPlus(trainMatrix, testMatrix, validMatrix, fold);
		case "timesvd++":
			return new TimeSVD(trainMatrix, testMatrix, fold);
		case "pmf":
			return new PMF(trainMatrix, testMatrix,validMatrix, fold);
			
		////////////////////////////////////////////////////////////////////
		case "tagcdcf":
			return new TagCDCF(trainMatrix, testMatrix, validMatrix, fold);	
		case "gtagcdcf":
			return new GTagCDCF(trainMatrix, testMatrix, validMatrix, fold);
		case "taggsvdpp":
			return new TagGSVDpp(trainMatrix, testMatrix, validMatrix, fold);
	
		case "tagembedsvd":
			return new TagEmbedSVD(trainMatrix, testMatrix, validMatrix, fold);
		case "tagembedsvd_sir":
			return new TagEmbedSVD_sir(trainMatrix, testMatrix, validMatrix, fold);
		case "tagembedsvdf":
			return new TagEmbedSVDF(trainMatrix, testMatrix, validMatrix, fold);
		case "tagembedsvdf_sir":
			return new TagEmbedSVDF_sir(trainMatrix, testMatrix, validMatrix, fold);
		...
		case "tagtrustsvd":
			return new TagTrustSVD(trainMatrix, testMatrix, validMatrix, fold);
		...
		case "lda":
			...
			/* extension */
		case "nmf":
			return new NMF(trainMatrix, testMatrix, validMatrix, fold);
		case "hybrid":
